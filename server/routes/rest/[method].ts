...
import { getReplayMix, listReplayYears, isReplayPlaylistId, parseReplayPlaylistId, replayPlaylistId, ALL_TIME_YEAR } from "~~/server/core/replayMix";
...
// --- Replay Mix, exposed read-only ------------------------------------------------------------------
//
// A Replay Mix is not a row in `playlists` (see server/core/replayMix.ts), so it needs its own id namespace
// here - "replay-2026", "replay-all-time" - and its own node builder. Subsonic clients get to browse and
// play it like any other playlist; what they don't get is any way to change it. Every mutating handler below
// rejects these ids outright, and since the ids can never resolve through getPlaylistById anyway, that
// rejection is a clear answer rather than the confusing "not found" a client would otherwise see.

// synthesizes the PlaylistRow shape the existing node mappers expect. `createdAt`/`updatedAt` come from the
// mix's refresh time so a syncing client sees the value change only when the mix actually changes.
function replayPlaylistRow(id: string, year: number, refreshedAt: number | null): PlaylistRow {
    const stamp = refreshedAt ?? 0;
    return {
        id,
        ownerId: "",
        title: year === ALL_TIME_YEAR ? "Replay Mix - All Time" : `Replay Mix ${year}`,
        description: "Automatically generated by Nafyn. Read-only.",
        privacy: "private",
        image: null,
        sortMode: "manual",
        createdAt: stamp,
...
async function replayNode(userId: string, year: number, refreshedAt: number | null, withSongs: boolean): Promise<SubsonicNode | null> {
    const mix = await getReplayMix(userId, year);
    if (mix.entries.length === 0) return null;

    // only tracks the user still holds can be played, so unavailable ones are left out of the Subsonic view
    // entirely rather than offered as entries that would fail to stream
    const playable = mix.entries.filter((e) => e.media !== null);
    const row = replayPlaylistRow(replayPlaylistId(year), year, refreshedAt ?? mix.refreshedAt);

    if (!withSongs) {
        const duration = playable.reduce((sum, e) => sum + (e.media?.duration ?? 0), 0);
        return playlistNode(row, playable.length, duration);
    }

    const filePathByMediaId = new Map<string, string>();
    for (const e of playable) {
        const source = await findAnyLibraryEntryForMedia(e.media!.id);
        if (source) filePathByMediaId.set(e.media!.id, source.filePath);
    }

    return playlistWithSongsNode(
        row,
        playable.map((e) => ({ media: e.media! })) as unknown as Parameters<typeof playlistWithSongsNode>[1],
        filePathByMediaId
    );
}

async function replayNodes(userId: string): Promise<SubsonicNode[]> {
    const years = await listReplayYears(userId);
    const nodes = await Promise.all(years.map((entry) => replayNode(userId, entry.year, entry.refreshedAt, false)));
    return nodes.filter((n): n is SubsonicNode => n !== null);
}
...
const handlers: Record<string, Handler> = {
    ...
    getPlaylists: async (_event, _query, user) => {
        ...
        return [el("playlists", {}, asList([...nodes, ...await replayNodes(user.id)]))];
    },

    getPlaylist: async (_event, query, user) => {
        const id = requireParam(query, "id");

        // resolved before getPlaylistById, which could never match a replay id anyway
        const replayYear = parseReplayPlaylistId(id);
        if (replayYear !== null) {
            const node = await replayNode(user.id, replayYear, null, true);
            if (!node) throw new SubsonicApiError(SubsonicErrors.notFound);
            return [node];
        }
    ...
    createPlaylist: async (_event, query, user) => {
        ...
        let playlist;
        if (existingId) {
            // "not authorized" rather than the "not found" the lookup below would give: the mix does exist,
            // it just cannot be written to, by this user or any other
            if (isReplayPlaylistId(existingId)) throw new SubsonicApiError(SubsonicErrors.notAuthorized);
    ...
    updatePlaylist: async (_event, query, user) => {
        ...
        if (isReplayPlaylistId(id)) throw new SubsonicApiError(SubsonicErrors.notAuthorized);
    ...
    scrobble: async (_event, query, user) => {
        ...
        // Subsonic distinguishes a "now playing" ping (submission=false) from a completed play; only the
        // latter is a listen
        ...
        // toward the user's insights - cross-device consistency is keyed on user_id alone, and this is the
