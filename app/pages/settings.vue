<template>
  <div class="settings">
    ...
    <section class="settings-content">
      ...
      <div v-else-if="activeCategory === 'import'" class="panel">
        <h1>{{ $t('transfer.title') }}</h1>
        <ImportPanel :permissions="perms" />
      </div>
...
import ImportPanel from '~/components/transfer/ImportPanel.vue';

const token = useCookie("nafynToken").value ?? "";

interface Category {
  id: "profile" | "accounts" | "storage" | "subsonic" | "privacy" | "appearance" | "playback" | "import" | "developer",
  label: string
}

// `?tab=import` lands straight on a panel - used by the import pages to link back here
const route = useRoute();
const activeCategory = ref<Category["id"]>(route.query.tab === 'import' ? 'import' : 'profile');
...
const categories = computed(() => {
  ...
  cats.push({ id: 'import', label: $t('settings.categories.import') });
